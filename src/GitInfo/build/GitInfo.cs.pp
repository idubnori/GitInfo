...
namespace _RootNamespace_
{
  ...
  partial class ThisAssembly
  {
	...
	internal static class UseGitInfoExtensions
	{
		public static Lazy<string> SerializedThisAssembly => new Lazy<string>(() =>
			JsonConvert.SerializeObject(typeof(ThisAssembly).GetFieldObjects(), Formatting.Indented));

		public static void UseGitInfo(this IApplicationBuilder app)
		{
			app.Map("/gitinfo", app1 =>
			{
				app1.Run(async context =>
				{
					await context.Response.WriteAsync(SerializedThisAssembly.Value);
				});
			});
		}

		private static Dictionary<string, object> GetFieldObjects(this Type target)
		{
			var fields = target.GetFields().ToDictionary(x => x.Name, x => x.GetValue(null));
			foreach (var nestedType in target.GetNestedTypes())
			{
				fields.Add(nestedType.Name, nestedType.GetFieldObjects());
			}
			return fields;
		}
	}
